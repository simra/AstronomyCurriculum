...
- After every full-package review result, immediately create or update `materials/<COURSECODE>/review-report.md` with the review date, scope, actual reviewer verdict, blocking findings, corrections and re-review outcome, residual caveats, and pre-instructional-use items. Do not leave an initial `Needs correction` report in place after the reviewer later approves, and do not leave a stale "pending review" report after recording approval.
- Treat the review report as a hard release gate, not a post-approval clerical task: even if the reviewer returns `Approved for review release`, the package is not complete and must not be committed/pushed or handed off until `review-report.md`, `course-manifest.json`, and `index.html` all exist and state the same current verdict. If any file is missing, says "pending", or contradicts the reviewer, synchronize it and validate again before declaring completion. The report must record the reviewer result that was actually returned; never infer approval from a manifest or index status.
...
- A template-reuse bug can also produce an index that has no leftover course-number strings yet still contains an entirely wrong course's actual lecture/lab titles and links (e.g., an instrumentation course's titles appearing under an exoplanets course's header). Before finalizing, open at least one linked lecture or lab file directly and confirm its own `<h1>` matches the title shown in the generated index; do not rely on a string search for the old course's number alone to catch this class of defect.
