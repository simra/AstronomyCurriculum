...
- Immediately after each full review, create/update `materials/<COURSECODE>/review-report.md` with date, scope, the actual reviewer verdict, findings, corrections/re-review outcome, residual caveats, and pre-instructional-use items.
- Treat the report as a hard release gate: do not call the course complete or commit/push until review-report.md, course-manifest.json, and index.html all exist and agree with the latest reviewer verdict. An approved agent response does not count as recorded approval while any artifact still says pending, needs correction, or not yet reviewed; synchronize and validate all statuses before handoff.
